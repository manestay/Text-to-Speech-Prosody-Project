...
def test(clf, X_train, y_train, X_test, y_test, columns):
    ...
    print("accuracy=%.3f (random=%.3f) f1=%.3f precision=%.3f recall=%.3f"%(
      ...
      sklearn.metrics.f1_score(y_test, y_pred, pos_label="B"),
      sklearn.metrics.precision_score(y_test, y_pred, pos_label="B"),
      sklearn.metrics.recall_score(y_test, y_pred, pos_label="B")))
